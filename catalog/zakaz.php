...
{
  echo "<script>
  alert(\"Оформить заказ могут лишь авторизованные пользователи!!!\");  
  window.location.href = '../php/login in.php'; 
  </script>";
}
?>
<!DOCTYPE html>
...
<form action="../php/orders.php" class="row black-text" method="post" enctype="multipart/form-data">   
    <div class="col-8">
...
<div class="col-8">
<button class="button1" type="submit">Заказать</button>
     <br>
     <br>
   </div>
</form>
...
<?php if (!empty($_SESSION['cart'])): ?>
  <table class="table">
            <tr>
                <td align="left">Товаров в корзине: <span id="modal-cart-qty"><?= $_SESSION['cart.qty'] ?></span>
                    <br> На сумму: <?= $_SESSION['cart.sum'] ?> р.
                </td>
            </tr>
            </tbody>
        </table>
